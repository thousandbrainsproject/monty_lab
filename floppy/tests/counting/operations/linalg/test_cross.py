...
def test_cross_basic() -> None:
    """Test cross product behavior and flop count with basic vectors."""
    counter = FlopCounter()
    with counter:
        a = np.array([1, 0, 0])
        b = np.array([0, 1, 0])
...
def test_cross_multiple() -> None:
    """Test cross product behavior and flop count with multiple vectors."""
    counter = FlopCounter()
    with counter:
        ...
        result = np.cross(a, b)
        assert counter.flops == 18  # noqa: PLR2004
        np.testing.assert_allclose(result, np.array([[0, 0, 1], [0, 0, 4]]))


def test_cross_broadcasting() -> None:
    """Test cross product behavior and flop count with broadcasting."""
    counter = FlopCounter()
    with counter:
