...
class CameraViewController: UIViewController, AVCaptureDataOutputSynchronizerDelegate {
    ...
    private var currentDepthImage: AVDepthData?
    
    private var currentVideoImage: CVPixelBuffer?
    
    private var currentJETImage: CVPixelBuffer?
...
//        // Save RGB data
//        let cirgbImage = CIImage(cvPixelBuffer: self.currentVideoImage!)
//        let rgbcontext = CIContext(options: nil)
//
//        guard let cgrgbImage = rgbcontext.createCGImage(cirgbImage, from: cirgbImage.extent) else {
//            print("Failed to create CGImage")
//            return
//        }
//
//        let uirgbImage = UIImage(cgImage: cgrgbImage)
//        UIImageWriteToSavedPhotosAlbum(uirgbImage, nil, nil, nil)
//
//        // Save JET data
//        let cijetImage = CIImage(cvPixelBuffer: self.currentJETImage!)
//        let jetcontext = CIContext(options: nil)
//
//        guard let cgjetImage = jetcontext.createCGImage(cijetImage, from: cijetImage.extent) else {
//            print("Failed to create CGImage")
//            return
//        }
//
//        let uijetImage = UIImage(cgImage: cgjetImage)
//        UIImageWriteToSavedPhotosAlbum(uijetImage, nil, nil, nil)
    ...
    func uploadRGBData() {
        // Get RGB image from video frame
        let cirgbImage = CIImage(cvPixelBuffer: self.currentVideoImage!)
        let rgbcontext = CIContext(options: nil)
        guard let cgrgbImage = rgbcontext.createCGImage(cirgbImage, from: cirgbImage.extent) else {
            print("Failed to create CGImage")
        ...
        // Create PNG image from RGB data
        let rgbData = uirgbImage.pngData()


        //
        // Upload RGB data to monty server
        //

        // Get server URL from settings
        guard let monty_url = UserDefaults.standard.string(forKey: "monty_url") else {
            print("Failed to get monty URL")
            return
        }
        let rgb_data_url = URL(string: monty_url + "/rgb.png")!

        // Create HTTP PUT request
        var request = URLRequest(url: rgb_data_url)
        ...
        // Send PUT request
        let task = URLSession.shared.uploadTask(with: request, from: rgbData) { data, response, error in
            if let error = error {
                print ("error: \(error)")
                return
            }
    ...
    func uploadDepthData() {

        // Get depth data from video frame
        guard let depthDataMap = self.currentDepthImage?.depthDataMap else {
            print("Failed to get depth data")
            return
        }
        let width = CVPixelBufferGetWidth(depthDataMap)
        let height = CVPixelBufferGetHeight(depthDataMap)
        let depthDataSize = height * width

        // Get pointer to depth data memory
        CVPixelBufferLockBaseAddress(depthDataMap, CVPixelBufferLockFlags.readOnly)
        let dephDataAddress = CVPixelBufferGetBaseAddress(depthDataMap)
        CVPixelBufferUnlockBaseAddress(depthDataMap, CVPixelBufferLockFlags.readOnly)

        // swift does not have an Float16 data type. Use UInt16 instead, and then translate
        let depthDataUInt16 = dephDataAddress!.assumingMemoryBound(to: UInt16.self)
        var depthDataFloat32 = [Float](repeating: 0, count: depthDataSize)
        var buf16 = vImage_Buffer(data: depthDataUInt16, height: 1, width: UInt(depthDataSize), rowBytes: depthDataSize * 2)
        var buf32 = vImage_Buffer(data: &depthDataFloat32, height: 1, width: UInt(depthDataSize), rowBytes: depthDataSize * 4)
        if vImageConvert_Planar16FtoPlanarF(&buf16, &buf32, 0) != kvImageNoError {
          print("Error converting float16 to float32")
        }
        let depthData = depthDataFloat32.withUnsafeBufferPointer({Data(buffer: $0)})

        //
        // Upload depth data to monty server
        //
        ...
        // Create HTTP PUT request
        let depth_data_url = URL(string: monty_url + "/depth.data")!
        var request = URLRequest(url: depth_data_url)
        request.httpMethod = "PUT"
        request.allHTTPHeaderFields = [
        ...
        // Send PUT request
        let task = URLSession.shared.uploadTask(with: request, from: depthData) { data, response, error in
            if let error = error {
                print ("error: \(error)")
                return
            }
    ...
    func dataOutputSynchronizer(_ synchronizer: AVCaptureDataOutputSynchronizer,
        ...
        self.currentDepthImage = depthData
        ...
        if JETEnabled {
            ...
            self.currentJETImage = jetPixelBuffer
            self.currentVideoImage = videoPixelBuffer
